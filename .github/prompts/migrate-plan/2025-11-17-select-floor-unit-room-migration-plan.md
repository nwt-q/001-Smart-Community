...
根据 `.github\prompts\route-migration-map.yml` 映射表：
...
3. **更新迁移映射表**：完成后在 `.github\prompts\route-migration-map.yml` 标记状态
...
| 验收项         | 验收标准                                                                         |
| :------------- | :------------------------------------------------------------------------------- |
| 迁移映射表更新 | `.github\prompts\route-migration-map.yml` 中 `选择器页面 (3个)` 部分已标记为完成 |
| 迁移计划文档   | 本文档已生成在 `.github\prompts\migrate-plan` 目录                               |
...
| Vue2 到 Vue3 路由迁移映射表 | `.github\prompts\route-migration-map.yml`   |
