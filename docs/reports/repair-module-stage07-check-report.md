...
| :--: | :------------------------------------ | :--------------------------- | :-------------: | :------: |
|  1   | `getRepairStates()` 函数未定义        | order-list.vue, dispatch.vue |  api-migration  | 30 分钟  |
|  2   | `getRepairStaffList()` 函数未定义     | dispatch.vue                 |  api-migration  | 30 分钟  |
|  3   | `getRepairStaffRecords()` 函数未定义  | order-detail.vue             |  api-migration  | 30 分钟  |
|  4   | `getRepairPayTypes()` 函数未定义      | handle.vue                   |  api-migration  | 30 分钟  |
|  5   | `getRepairResources()` 函数未定义     | select-resource.vue          |  api-migration  | 30 分钟  |
|  6   | `getRepairResourceTypes()` 函数未定义 | select-resource.vue          |  api-migration  | 30 分钟  |
|  7   | `handle.vue:261` 使用不存在的路由方法 | handle.vue                   | route-migration |  5 分钟  |
...
| 序号 | 问题                                 | 影响范围     |    来源检查     | 修复时间 |
| :--: | :----------------------------------- | :----------- | :-------------: | :------: |
|  8   | API 参数类型不一致                   | 多个页面     |  api-migration  | 1-2 小时 |
|  9   | `dispatch.vue:224` action 参数不正确 | dispatch.vue | route-migration | 10 分钟  |
|  10  | `dispatch.vue:249-261` 参数不匹配    | dispatch.vue | route-migration | 15 分钟  |
|  11  | `finishRepair` 使用 `any[]`          | handle.vue   | code-migration  | 10 分钟  |
|  12  | 错误处理中使用 `any`                 | 多个页面     | code-migration  |  1 小时  |
